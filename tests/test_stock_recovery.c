...
typedef struct {
  ...
  RenameCrashPoint
      crash_point; /* consumed (reset to CrashNone) by the next rename() call */
...
static void fake_port_init(FakePort *fp) { memset(fp, 0, sizeof(*fp)); }
...
static void fake_set_bytes(FakePort *fp, const char *path, const uint8_t *data,
                           size_t len) {
...
static void fake_set_items(FakePort *fp, const char *path,
                           const StockItem *items, size_t count) {
  fake_set_bytes(fp, path, (const uint8_t *)items, count * sizeof(StockItem));
}

static StoreStat fake_stat(void *ctx, const char *path) {
  FakePort *fp = ctx;
  if (fp->force_stat_error_path[0] &&
      strcmp(fp->force_stat_error_path, path) == 0) {
...
static StockReadOutcome fake_read_all(void *ctx, const char *path,
                                      StockItem **out_items,
                                      size_t *out_count) {
  FakePort *fp = ctx;
  if (fp->force_blocked_read_path[0] &&
      strcmp(fp->force_blocked_read_path, path) == 0) {
...
static bool fake_write_all(void *ctx, const char *path, const StockItem *items,
                           size_t count) {
  FakePort *fp = ctx;
  if (fp->force_write_fail_path[0] &&
      strcmp(fp->force_write_fail_path, path) == 0) {
...
static bool fake_read_raw(void *ctx, const char *path, uint8_t **out_bytes,
                          size_t *out_len) {
  FakePort *fp = ctx;
  if (fp->force_blocked_read_path[0] &&
      strcmp(fp->force_blocked_read_path, path) == 0) {
...
static void test_corrupt_then_upsert_keeps_original_in_bad(void) {
  ...
  assert(bad && bad->len == sizeof(garbage) &&
         memcmp(bad->data, garbage, sizeof(garbage)) == 0);

  StockItem *items = NULL;
  size_t count = 0;
  const StockReadOutcome final_ro =
      fake_read_all(&fp, MAIN_PATH, &items, &count);
...
static void test_second_corruption_lands_in_bad1_keeps_first(void) {
  ...
  FakeFile *bad = fake_find(&fp, BAD_PATH);
  assert(bad && bad->len == sizeof(first) &&
         memcmp(bad->data, first, sizeof(first)) == 0);
  FakeFile *bad1 = fake_find(&fp, BAD1_PATH);
  assert(bad1 && bad1->len == sizeof(second) &&
         memcmp(bad1->data, second, sizeof(second)) == 0);
...
static void test_all_backup_slots_full_blocks_without_overwriting(void) {
  ...
  assert(main_file && main_file->len == sizeof(garbage) &&
...
static void test_quarantine_rename_failure_is_distinct_from_slots_full(void) {
  ...
  printf(
      "test_quarantine_rename_failure_is_distinct_from_slots_full: PASSED\n");
}
...
static void test_read_error_blocks_without_quarantine(void) {
  ...
  strncpy(fp.force_blocked_read_path, MAIN_PATH,
          sizeof(fp.force_blocked_read_path) - 1);
...
static void test_crash_mid_save_recovers_from_tmp(void) {
  ...
  StockItem item_a = make_item(0xAA, "A");
  assert(stock_recovery_upsert(&port, MAIN_PATH, &item_a).outcome ==
         StockWriteOk);
  ...
  StockItem *items = NULL;
  size_t count = 0;
  StockLoadResult recovered =
      stock_recovery_load(&port, MAIN_PATH, &items, &count);
...
static void
test_tmp_read_error_blocks_without_quarantine_when_main_missing(void) {
  FakePort fp;
  fake_port_init(&fp);
  StockStoragePort port;
  fake_port_bind(&fp, &port);
  ...
  strncpy(fp.force_blocked_read_path, TMP_PATH,
          sizeof(fp.force_blocked_read_path) - 1);
...
static void test_promote_then_next_write_failure_preserves_promoted_data(void) {
  ...
  fp.crash_point = CrashAfterRemoveDst;
  StockItem item_b = make_item(0xBB, "B");
  assert(stock_recovery_upsert(&port, MAIN_PATH, &item_b).outcome ==
         StockWriteFailed);
  assert(!fake_exists(&fp, MAIN_PATH));
  assert(fake_exists(&fp, TMP_PATH));

  strncpy(fp.force_write_fail_path, TMP_PATH,
          sizeof(fp.force_write_fail_path) - 1);
  ...
  printf(
      "test_promote_then_next_write_failure_preserves_promoted_data: PASSED\n");
}
...
static void test_zero_byte_main_after_crash_promotes_tmp(void) {
  ...
  fake_set_bytes(&fp, MAIN_PATH, NULL,
                 0); /* an empty file is a trivial byte-prefix too */
...
static void test_main_blocked_never_promotes_tmp_over_it(void) {
  ...
  const StockItem ab[] = {a, b};
  fake_set_items(&fp, MAIN_PATH, ab, 2);
  fake_set_bytes(&fp, TMP_PATH, NULL,
                 0); /* leftover empty tmp: would parse "fine" */
  strncpy(fp.force_blocked_read_path, MAIN_PATH,
          sizeof(fp.force_blocked_read_path) - 1);
...
static void test_main_stat_error_never_promotes_tmp_over_it(void) {
  ...
  strncpy(fp.force_stat_error_path, MAIN_PATH,
          sizeof(fp.force_stat_error_path) - 1);
...
static void test_oversized_main_quarantined_before_tmp_promotes(void) {
  ...
  assert(items[0].uid[0] == 0xA1);
  assert(
      survives_anywhere(&fp, 0xB2)); /* preserved inside the quarantined .bad */
  free(items);
...
static void test_recover_after_remove_then_partial_promote(void) {
  ...
  assert(stock_recovery_upsert(&port, MAIN_PATH, &a).outcome == StockWriteOk);
  fp.crash_point = CrashAfterRemoveDst;
  assert(stock_recovery_upsert(&port, MAIN_PATH, &b).outcome ==
         StockWriteFailed);
...
static void test_promote_crash_after_remove_dst_blocks_and_preserves_b(void) {
  ...
  printf(
      "test_promote_crash_after_remove_dst_blocks_and_preserves_b: PASSED\n");
}
...
static void test_promote_crash_after_copy_blocks_and_preserves_b(void) {
  ...
  assert(r.outcome == StockWriteBlockedIoError);
  assert(file_record_count(&fp, MAIN_PATH) ==
         2); /* the copy already completed */
  assert(survives_anywhere(&fp, 0xB2));
...
static void test_tmp_stat_error_blocks_and_preserves_b(void) {
  ...
  fake_set_items(&fp, MAIN_PATH, only_a, 1);
  fake_set_items(&fp, TMP_PATH, ab, 2);
  strncpy(fp.force_stat_error_path, TMP_PATH,
          sizeof(fp.force_stat_error_path) - 1);
