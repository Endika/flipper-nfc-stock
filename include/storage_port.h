...
typedef enum {
  StockReadOk =
      0, /* whole file is complete StockItem records (possibly zero of them) */
...
typedef struct {
  ...
  StockReadOutcome (*read_all)(void *ctx, const char *path,
  ...
  bool (*read_raw)(void *ctx, const char *path, uint8_t **out_bytes,
                   size_t *out_len);
  bool (*write_all)(void *ctx, const char *path, const StockItem *items,
                    size_t count);
