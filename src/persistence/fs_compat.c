...
StockReadOutcome fs_read_all_stock_items_ex(const char *path,
                                            StockItem **out_items,
                                            size_t *out_count) {
  *out_items = NULL;
  *out_count = 0;
...
bool fs_exists(const char *path) { return access(path, F_OK) == 0; }

bool fs_rename(const char *src, const char *dst) {
  return rename(src, dst) == 0;
}

bool fs_remove(const char *path) { return remove(path) == 0; }
