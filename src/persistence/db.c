...
static StockReadOutcome port_read_all(void *ctx, const char *path,
                                      StockItem **out_items,
                                      size_t *out_count) {
  (void)ctx;
...
static bool port_write_all(void *ctx, const char *path, const StockItem *items,
                           size_t count) {
  (void)ctx;
...
static bool port_read_raw(void *ctx, const char *path, uint8_t **out_bytes,
                          size_t *out_len) {
  (void)ctx;
