...
static void nfc_stock_notice_for_write(const StockWriteResult *result,
  ...
  case StockWriteBlockedQuarantineFailed:
    snprintf(buf, buf_size,
             "Couldn't protect your saved data. Changes won't be saved.");
    return;
  case StockWriteBlockedIoError:
    snprintf(buf, buf_size,
             "Couldn't read your saved data. Changes won't be saved.");
    return;
  case StockWriteRefusedOverLimit:
    snprintf(buf, buf_size,
             "That would make the database too large. Not saved.");
    return;
...
static void nfc_stock_show_write_notice(NfcStockApp *app, const char *message) {
  ...
  widget_add_text_box_element(app->ui->widget, 0, 0, 128, 64, AlignLeft,
                              AlignTop, message, false);
...
static bool nfc_stock_details_save_and_maybe_notify(NfcStockApp *app) {
  ...
  StockWriteResult result =
      stock_db_upsert(app->active_db_path, &app->current_item);
  if (result.outcome != StockWriteOk &&
      result.outcome != StockWriteOkQuarantined) {
...
void nfc_stock_scene_edit_on_exit(void *context) { UNUSED(context); }
...
static void nfc_stock_inventory_action_callback(void *context, uint32_t index) {
  ...
  if (index == InvRow_Edit) {
    ...
    StockWriteResult result =
        stock_db_delete_at(app->active_db_path, selected_index);
    char message[NFC_STOCK_NOTICE_MAX];
    nfc_stock_notice_for_write(&result, message, sizeof(message));
    if (message[0] != '\0') {
      nfc_stock_show_write_notice(app, message);
...
static void nfc_stock_settings_activate_db(NfcStockApp *app,
                                           const char *new_path) {
...
static void nfc_stock_settings_apply_db_name(NfcStockApp *app) {
  ...
  if (s_db_action == SettingsDbActionRename) {
    if (target_exists) {
      nfc_stock_show_write_notice(app,
