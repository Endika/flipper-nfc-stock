 ...
 * worst case. */
static void backup_path_for_slot(const char *main_path, int slot, char *out,
                                 size_t out_size) {
...
static QuarantineOutcome quarantine_file(const StockStoragePort *port,
...
static StockLoadResult
reconcile_main_with_tmp(const StockStoragePort *port, const char *main_path,
                        const char *tmp_path, StockItem *main_items,
                        size_t main_count, StockItem **out_items,
                        size_t *out_count) {
  ...
  const bool main_is_prefix =
      tmp_raw_len >= main_len &&
...
StockLoadResult stock_recovery_load(const StockStoragePort *port,
                                    const char *main_path,
                                    StockItem **out_items, size_t *out_count) {
  StockLoadResult result = {0};
  *out_items = NULL;
  *out_count = 0;
  ...
  bool main_confirmed_bad =
      false; /* Framing or TooLarge: known corrupt, never "unknown" */
  if (main_stat == StoreStatOk) {
    StockItem *main_items = NULL;
    size_t main_count = 0;
    const StockReadOutcome ro =
        port->read_all(port->ctx, main_path, &main_items, &main_count);
    if (ro == StockReadOk) {
      return reconcile_main_with_tmp(port, main_path, tmp_path, main_items,
                                     main_count, out_items, out_count);
    }
  ...
  if (tmp_stat == StoreStatOk) {
    const StockReadOutcome ro =
        port->read_all(port->ctx, tmp_path, &tmp_items, &tmp_count);
    if (ro == StockReadOk) {
      tmp_parsed_ok = true;
  ...
  if (tmp_parsed_ok) {
    if (main_confirmed_bad) {
      ...
      if (port->rename(port->ctx, tmp_path, main_path)) {
        ...
        *out_count = tmp_count;
        result.status =
            StoreLoadCorrupt; /* main was bad; still worth telling the user */
 ...
 * immediately. */
static bool stock_recovery_save(const StockStoragePort *port,
                                const char *main_path, const StockItem *items,
                                size_t count) {
  char tmp_path[STOCK_PATH_MAX];
  tmp_path_for(main_path, tmp_path, sizeof(tmp_path));
...
static bool merge_by_uid(const StockItem *items, size_t count,
                         const StockItem *item, StockItem **out_items,
                         size_t *out_count) {
  size_t idx = (size_t)-1;
...
StockWriteResult stock_recovery_upsert(const StockStoragePort *port,
                                       const char *main_path,
  ...
  const bool merged_ok =
      merge_by_uid(items, count, item, &merged, &merged_count);
  free(items);
  if (!merged_ok) {
  ...
  result.outcome =
      load.status == StoreLoadCorrupt ? StockWriteOkQuarantined : StockWriteOk;
  return result;
}

StockWriteResult stock_recovery_delete_at(const StockStoragePort *port,
                                          const char *main_path, size_t index) {
  StockWriteResult result = {StockWriteFailed, {0}};
  ...
  memmove(&items[index], &items[index + 1],
          (count - index - 1) * sizeof(StockItem));
